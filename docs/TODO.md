...
- [x] 전역 에러 핸들링
  - [x] `app/error.tsx` 생성
    - [x] Next.js Error Boundary 컴포넌트 구현
    - [x] `Error` 컴포넌트 재사용 (`components/ui/error.tsx`)
    - [x] 에러 타입별 메시지 분류 (네트워크, API, 일반)
    - [x] 재시도 기능 (`reset` 함수 사용)
    - [x] 홈으로 돌아가기 버튼
    - [x] 개발 환경에서 에러 로깅 (console.error)
    - [x] 사용자 친화적인 에러 메시지 표시
    - [x] 개발 환경에서만 상세 에러 정보 표시 (스택 트레이스, digest)
  - [x] `app/global-error.tsx` 생성
    - [x] 루트 레이아웃 레벨 에러 처리
    - [x] 레이아웃 자체의 에러를 처리 (ClerkProvider, SyncUserProvider 등)
    - [x] 최소한의 UI만 포함 (레이아웃이 깨진 경우 대비)
    - [x] HTML 구조 직접 작성 (레이아웃 컴포넌트 사용 불가)
    - [x] 인라인 스타일 사용 (Tailwind CSS 의존성 제거)
    - [x] 에러 로깅 및 사용자 안내 메시지
    - [x] 개발 환경에서만 상세 에러 정보 표시
  - [x] API 에러 처리 개선
    - [x] 에러 타입 분류 개선 (`classifyError` 함수)
      - [x] 네트워크 에러 감지 (fetch 실패, 타임아웃)
      - [x] API 응답 에러 감지 (HTTP 상태 코드별 처리)
      - [x] 데이터 파싱 에러 감지
      - [x] 에러 메시지 표준화
    - [x] 에러 메시지 개선
      - [x] 사용자 친화적인 한국어 메시지
      - [x] HTTP 상태 코드별 맞춤 메시지 (400, 401, 403, 404, 429, 500, 503)
      - [x] 한국관광공사 API 에러 코드별 메시지 (01~05)
      - [x] 기술적 에러 코드는 개발 환경에서만 표시
    - [x] 에러 로깅
      - [x] 개발 환경에서 상세 에러 로깅 (에러 타입, 메시지, 스택)
      - [x] 재시도 로직 개선 (네트워크 에러 및 5xx 에러만 재시도)
- [x] 404 페이지
